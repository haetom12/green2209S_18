...
@ServerEndpoint("/chatserverDb")
public class ChatServerDb {
...
//			
  	System.out.println("유져1 : " + user);
		System.out.println("텍트스1 : " + txt);		
...
//		}
		...
		else if (no.equals("4")) {  // 접속된 누군가가 '메세지'를 전송할때(대화할때 : '2유저명#메세지' 로 넘어온다.)
...
//			String[] order = txt.split("@");
			...
			for (Session s : list) {
				//if (s != session) { // 현재 접속자(메세지를 보낸 접속자)가 아닌 나머지 접속자들한테 아래 메세지를 보낼 수 있도록 한다.
					try {
						s.getBasicRemote().sendText("4#" + user + ":" + txt);
					} catch (IOException e) {
						e.printStackTrace();
					}
				//}
