...
<head>
  ...
  <script>
	  ...
	  //  체크후 서버로 전송(submit)
	  function fCheck() {
		  let subMenuName = myform.subMenuName.value;
		  let price = myform.price.value;
		  
			if(subMenuName.trim() == "") {
				alert("메뉴명을 입력하세요");
				myform.subMenuName.focus();
				return false;
			}
			else if(price.trim() == "") {
				alert("가격을 입력하세요");
			...
			else if(subMenuName != '${sVo.subMenuName}') {
				if(foodCheckSw == 0) {
	  ...
	  function foodNameCheck() {
	  	...
	  	if(subMenuName.trim() == "") {
		  	...
		  	$.ajax({
			  	type   : "post",
			  	url    : "${ctp}/store/subMenuNameCheck",
			  	data   : {
			  	...
			  	success:function(res) {
			  		if(res == "1") {
			  			...
			  			$("#subMenuName").attr("readonly","readonly");
			  			foodCheckSw = 1;
			  		}
			  	},
			  	error : function() {
			...
			<div class="container" >
				<section class="py-5 overflow-hidden" >
			        ...
			        <div class="loginbox bg-light">
			            <div class="fw-bold text-danger fs-3 fs-lg-5 lh-sm my-2 mt-2 text-center"><font size="20pt">추가메뉴 수정</font></div>
			            <div class="loginbox-or">
			                <div class="or-line"></div>
			            </div>
			            <div class="loginbox-textbox fw-bold text-danger fs-3 fs-lg-1 lh-sm" style="margin-left: 7%;">음식 태그</div>
			            <div class="loginbox-textbox">
			                <input type="text" class="form-control" id="foodTag" name="foodTag" value="${sVo.foodTag}" style="width: 85%; margin: 0 auto;" readonly required>
			            </div>
			            <div class="loginbox-textbox fw-bold text-danger fs-3 fs-lg-1 lh-sm" style="margin-left: 7%;">추가메뉴 이름</div>
			            <div class="loginbox-textbox input-group" style="width: 85.5%; margin: 0 auto;">
			                <input type="text" class="form-control" id="subMenuName" name="subMenuName" value="${sVo.subMenuName}" style="width: 100px;" required>&nbsp;
			            ...
			            <div class="loginbox-textbox">
			                <input type="number" class="form-control" id="price" name="price" value="${sVo.price}"  style="width: 85%; margin: 0 auto;" required>
			            </div>
		                ...
		                <input type="button" onclick="location.href='${ctp}/store/myStoreSubMenu?foodTag=${sVo.foodTag}';" class="btn btn-primary" value="돌아가기" style="width: 22%;  margin-left: 15%;">
			            </div>
			            <input type="hidden" name="storeName" value="${sVo.storeName}" />
			            <input type="hidden" name="subMenuIdx" value="${sVo.subMenuIdx}" />
			        </div>
		        </form>
		    </div>
			</section>
